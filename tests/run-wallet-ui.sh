...
# del contraste y el recorte en la vista de billetera.
...
BUILD="${BUILD_DIR:-dist-wallet-test}"
unset NODE_OPTIONS
...
if [ "${SKIP_BUILD:-0}" != "1" ]; then
  ...
  PATH="$NODE_DIR:$PATH" PARCEL_WORKERS=0 npx parcel build src/dapp.html \
    --dist-dir "$BUILD" --public-url ./ --cache-dir .parcel-cache-wallet-test > /tmp/build-wallet.log 2>&1 || {
      echo "✗ falló el build — mirá /tmp/build-wallet.log"; tail -20 /tmp/build-wallet.log; exit 1; }
fi
...
python3 -m http.server "$WEB_PORT" --bind 127.0.0.1 --directory "$BUILD" > /tmp/web-wallet.log 2>&1 &
WEB_PID=$!
sleep 1
...
"$CHROME" --headless=new --remote-debugging-port="$CDP_PORT" --user-data-dir="$PROF" \
  --no-first-run --window-size=1360,900 about:blank > /tmp/chrome-wallet.log 2>&1 &
CH_PID=$!
...
CDP="http://127.0.0.1:${CDP_PORT}" BASE_URL="http://localhost:${WEB_PORT}" "$NODE_BIN" tests/wallet-contraste-y-recorte-cdp.mjs
